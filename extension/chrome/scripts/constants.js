            ...
            detectionFunctions: [
                function (node, nodeOld) {
                    ...
                    if (nodeOld && node.innerText != nodeOld.innerText) {
                         ...
                         * @constant
                         */
                        const reg = /(?:\d{1,2}\s*:\s*){1,3}\d{1,2}|(?:\d{1,2}\s*(?:days?|hours?|minutes?|seconds?|[a-zA-Z]{1,3}\.?)(?:\s*und)?\s*){2,4}/gi;
                         ...
                         * @constant
                         */
                        const regBad = /(?:\d{1,2}\s*:\s*){4,}\d{1,2}|(?:\d{1,2}\s*(?:days?|hours?|minutes?|seconds?|[a-zA-Z]{1,3}\.?)(?:\s*und)?\s*){5,}/gi;
        ...
        },
        {
            ...
            detectionFunctions: [
                function (node, nodeOld) {
                    ...
                    // The previous state of the element is not used.
                    if (/(?:(?:₹|\$)\s*\d+(?:\.\d{2})?|\d+(?:\.\d{2})?\s*(?:rupees?|₹|dollars?))\s*(?:(?:(?:per|\/|a)\s*month)|(?:p|\/)m)\s*(?:after|from\s*(?:month|day)\s*\d+)/i.test(node.innerText)) {
            
                        return true;
                    }
                    if (/(?:(?:₹|rupees?|\$|dollars?)\s*\d+(?:\.\d{2})?)\s*(?:after\s*(?:the)?\s*\d+(?:th|nd|rd|th)?\s*(?:months?|days?)|from\s*(?:month|day)\s*\d+)/i.test(node.innerText)) {
            
                        return true;
                    }
                    if (/(?:after\s*that|then|afterwards|subsequently)\s*(?:(?:₹|\$)\s*\d+(?:\.\d{2})?|\d+(?:\.\d{2})?\s*(?:rupees?|₹|dollars?))\s*(?:(?:(?:per|\/|a)\s*month)|(?:p|\/)m)/i.test(node.innerText)) {
                    
                        return true;
                    }
                    if (/after\s*(?:the)?\s*\d+(?:th|nd|rd|th)?\s*months?\s*(?:only|just)?\s*(?:(?:₹|\$)\s*\d+(?:\.\d{2})?|\d+(?:\.\d{2})?\s*(?:rupees?|₹|dollars?))/i.test(node.innerText)) {
